...
void enter_power_down()
{
	...
	unsigned addr;
	unsigned gate;
	unsigned scan_code, custom_code, power_key;
...
//backup the remote config (on arm)
    ...
    while(1)
    {
        ...
        if(((test_status_0 = readl(P_AO_IR_DEC_STATUS))>>3) & 0x1){
        	scan_code = readl(P_AO_IR_DEC_FRAME);
			custom_code = scan_code & 0xffff;
        	power_key = (scan_code>>16)&0xffff;

        	if(((custom_code == 0xfe01)&&(power_key == 0xbf40))||((custom_code == 0xff00)&&(power_key == 0xa55a))){	
        	    //poweronflag = 1;
    		    //for(i = 0;i<1000;i++)
    		    //{
